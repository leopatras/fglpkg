...
|---|---|---|---|---|
| `fglpkg deprecate <pkg>@<ver>` (with `--moved-to <new>`) | ✅ Shipped (GIS-247) | — | M | Marks a version deprecated with a message, warned at install/`info`/`outdated` (npm model). `--moved-to <new>` records a successor package — this **absorbs the dropped `migrate` command** (rename/redirect). GI columns + flag route + `deprecated`/`moved_to` read fields are live; the **CLI command + consumer surfacing are now implemented** ([internal/cli/deprecate.go](../internal/cli/deprecate.go)). **Design:** [GI/registry side](../specs/gi-registry-workstream-c.md) §1 · [CLI side](../specs/deprecate-cli.md) |
...
| 11 | `deprecate` (+ `--moved-to` rename) | P1 | ✅ Shipped | Workstream C; absorbs the dropped `migrate` (#31) via `--moved-to`. CLI command + consumer surfacing (install/info/outdated) implemented (GIS-247). Specs: [GI](../specs/gi-registry-workstream-c.md) §1 + [CLI](../specs/deprecate-cli.md) |
